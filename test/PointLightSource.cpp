...
PointLightSource::PointLightSource(P3 position, Color color, double diffuseCoef, double specularCoef, double specularExponent) : LightSource(color, diffuseCoef) {
	this->position = position;
	this->specularCoef = specularCoef;
	this->specularExponent = specularExponent;
}

PointLightSource::PointLightSource(P3 position, Color color) : LightSource(color) {
	this->position = position;
...
Color PointLightSource::getReflection(P3 & surfPoint, P3 & normalVec, P3 & observerPosition, Color & objectColor) {
	...
	double costheta = lightVector * normalVec;
	Color cDiff = diffuseCoef * (objectColor % color) * costheta;
	// specular
	...
	sym.normalize();
	double coskSO = pow(sym * obsVec, specularExponent);
	Color cSpec = specularCoef * color * coskSO;
	return cDiff + cSpec;
}

/*
Color PointLightSource::getReflection(P3 & surfPoint, P3 & normalVec, P3 & observerPosition, Color & objectColor, Scene * scene) {
